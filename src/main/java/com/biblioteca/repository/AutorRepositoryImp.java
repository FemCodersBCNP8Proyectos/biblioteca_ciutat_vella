...
public class AutorRepositoryImp implements AutorRepository {
    ...
    public List<Autor> selectAutorByName(String nombre) {
        ...
        try (Connection conn = DBManager.getConnection();
            ...
            throw new RuntimeException(
                    Colors.RED + "Error al buscar autor/es por nombre: " + e.getMessage() + Colors.RESET);
        }
        return autores;
    }
    ...
    public void updateAutor(Autor autor) {
        ...
        try (Connection conn = DBManager.getConnection();
            ...
            if (rows > 0) {
                ...
                System.out.println(
                        Colors.RED + "No se encontró ningún autor con ID: " + autor.getId_autor() + Colors.RESET);
            }
        } catch (SQLException e) {
    ...
    public void deleteAutorById(Integer id_autor) {
        ...
        if (!librosAutor.isEmpty()) {
            System.out.println(
                    Colors.RED + "ACCIÓN DENEGADA: El autor tiene libros vinculados: " + librosAutor + Colors.RESET);
            return;
        }
        ...
        try (Connection conn = DBManager.getConnection();
            ...
            st.setInt(1, id_autor);
            st.executeUpdate();
            System.out.println(Colors.GREEN + "Autor eliminado correctamente." + Colors.RESET);
        } catch (SQLException e) {
            throw new RuntimeException(Colors.RED + "Error al borrar: " + e.getMessage() + Colors.RESET);
        }
    }
    ...
    public void deleteAutorByName(String nombre) {
        ...
        try (Connection conn = DBManager.getConnection();
            ...
            try (ResultSet rs = st.executeQuery()) {
                if (rs.next()) {
                    deleteAutorById(rs.getInt("id_autor"));
                } else {
                    System.out.println(Colors.YELLOW + "El autor '" + nombre + "' no existe." + Colors.RESET);
                }
            }
        } catch (SQLException e) {
    ...
    private List<String> findAutorLibros(Integer id_autor) {
        ...
        try (Connection conn = DBManager.getConnection();
            ...
            throw new RuntimeException(
                    Colors.RED + "Error al consultar libros del autor: " + e.getMessage() + Colors.RESET);
        }
        return librosAutor;
    }
